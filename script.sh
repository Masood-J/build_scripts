...
. build/envsetup.sh
echo "====== Envsetup Done ======="
cat > device/samsung/a20/genesis_a20.mk << 'EOF'
...
PRODUCT_MAKEFILES := \
    device/samsung/a20/genesis_a20.mk
COMMON_LUNCH_CHOICES := \
    genesis_a20-eng \
    genesis_a20-user \
    genesis_a20-userdebug
EOF

cat > device/samsung/a20e/genesis_a20e.mk << 'EOF'
# Copyright (C) 2018 The LineageOS Project
# SPDX-License-Identifier: Apache-2.0
# Inherit from those products. Most specific first.
$(call inherit-product, $(SRC_TARGET_DIR)/product/product_launched_with_p.mk)
...
PRODUCT_MAKEFILES := \
    device/samsung/a20e/genesis_a20e.mk
COMMON_LUNCH_CHOICES := \
    genesis_a20e-eng \
    genesis_a20e-user \
    genesis_a20e-userdebug
EOF

# Build for A10
lunch genesis_a20-user && make installclean && mka genesis && lunch genesis_a20e-user && make installclean && mka genesis
